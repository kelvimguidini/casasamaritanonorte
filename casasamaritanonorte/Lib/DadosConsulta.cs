        ...
        { 
            ...
            if (retornoCorreios.Servicos.Length > 0)
            {
                if (retornoCorreios.Servicos[0].Erro == "0")
                {
                    ...
                    return JsonConvert.SerializeObject(new { valor = retornoCorreios.Servicos[0].Valor, prazo = retornoCorreios.Servicos[0].PrazoEntrega + 2 });
                }
                else
                {
                    return JsonConvert.SerializeObject(new { erro = retornoCorreios.Servicos[0].Erro, mensagem = retornoCorreios.Servicos[0].MsgErro });
                }
            }
            else
            {
                return JsonConvert.SerializeObject(new { erro = 1, mensagem = "Não foi possível efetuar a consulta" });
            }
        }
